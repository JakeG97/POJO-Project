...
function keyInObjectArray(objArray, keyString) {
  for (let i = 0; i < objArray.length; i++) {           // we need to iterate through with a normal FOR loop
    if (Object.keys(objArray[i]).includes(keyString)) { // conditional that checks if the keyString is included within the arr
      return true
    }
  }
  return false
